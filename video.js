...
/* Stream it to the video element */
camera();
...
// Function to speak out the detected object
function speakObject(objectName) {
    // Check if the object has already been spoken
    if (!spokenObjects.includes(objectName)) {
        // Create a new speech utterance
        var utterThis = new SpeechSynthesisUtterance(objectName);

        // Speak the utterance
        synth.speak(utterThis);

        // Add the object to the spokenObjects list
        spokenObjects.push(objectName);
    }
}
